...
4. Create a client secret for authentication and store it securely.
   - On the same Agent Blueprint details page, goto `Credentials` > `Client secrets` > `New client secret`, save the `value` securely, it will be used in the next step to configure the agent service.
